function [spiketimes_e, spiketimes_i] = gamma_simulator_plots(pert, paramsfile, plotname)
% gamma_simulator.m
...
for k=1:m_steps,
	...
	n_e_inc=(n_e_inf(v_e, pert(3), pert(4))-n_e)./tau_n_e(v_e, pert(3), pert(4));
    h_e_inc=(h_e_inf(v_e, pert(5), pert(6))-h_e)./tau_h_e(v_e, pert(5), pert(6));
	...
	n_i_inc=(n_i_inf(v_i, pert(3), pert(4))-n_i)./tau_n_i(v_i, pert(3), pert(4));
    h_i_inc=(h_i_inf(v_i, pert(5), pert(6))-h_i)./tau_h_i(v_i, pert(5), pert(6));
    s_i_inc=0.5*(1+tanh(v_i/4)).*(1-s_i)./tau_r_i-s_i./tau_d_i;

	v_e_tmp=v_e+dt05*v_e_inc;
	n_e_tmp=n_e+dt05*n_e_inc;
	m_e_tmp=m_e_inf(v_e_tmp, pert(1), pert(2));
    h_e_tmp=h_e+dt05*h_e_inc;
    s_e_tmp=s_e+dt05*s_e_inc;    
	v_i_tmp=v_i+dt05*v_i_inc;
	n_i_tmp=n_i+dt05*n_i_inc;
	m_i_tmp=m_i_inf(v_i_tmp, pert(1), pert(2));
	...
	n_e_inc=(n_e_inf(v_e_tmp, pert(3), pert(4))-n_e_tmp)./tau_n_e(v_e_tmp, pert(3), pert(4));
    h_e_inc=(h_e_inf(v_e_tmp, pert(5), pert(6))-h_e_tmp)./tau_h_e(v_e_tmp, pert(5), pert(6));
	...
	n_i_inc=(n_i_inf(v_i_tmp, pert(3), pert(4))-n_i_tmp)./tau_n_i(v_i_tmp, pert(3), pert(4));
    h_i_inc=(h_i_inf(v_i_tmp, pert(5), pert(6))-h_i_tmp)./tau_h_i(v_i_tmp, pert(5), pert(6));
	...
	v_e=v_e+dt*v_e_inc;
	n_e=n_e+dt*n_e_inc;
	m_e=m_e_inf(v_e, pert(1), pert(2));
    h_e=h_e+dt*h_e_inc;
    s_e=s_e+dt*s_e_inc;
	v_i=v_i+dt*v_i_inc;
	n_i=n_i+dt*n_i_inc;
	m_i=m_i_inf(v_i, pert(1), pert(2));
    h_i=h_i+dt*h_i_inc;
    s_i=s_i+dt*s_i_inc;
    s_stoch_e=s_stoch_e*r_e; s_stoch_i=s_stoch_i*r_i;
...
% spiketimes_e=spiketimes_e(spiketimes_e(:,1)>50.0,:);
% spiketimes_i=spiketimes_i(spiketimes_i(:,1)>50.0,:);
% nisis=diff(spiketimes_e(:,1));
% nisis=nisis(nisis>0.1);
% avg_freq=1/(mean(nisis)/1000);
